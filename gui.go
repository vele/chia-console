...
func banner(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	if v, err := g.SetView("banner", 0, int(float32(maxY)/1.6)+1, int(float32(maxX)-1.5), int(float32(maxY)/1.5)); err != nil {
...
func leftTop(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	if v, err := g.SetView("diskspace", 0, 0, int(float32(maxX)/4), int(float32(maxY)/4)); err != nil {
		if err != gocui.ErrUnknownView {
			log.Fatal("POOP")
			return err
		}
...
func secondRowGraph(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	//int(float32(maxY) / 2)
	if v, err := g.SetView("totalPlots", int(float32(maxX)/3)+1, maxY/4+1, maxX-10, int(float32(maxY)/2)); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
...
func firstRowGraph(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	if v, err := g.SetView("main", int(float32(maxX)/3)+1, 0, maxX-10, maxY/4); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
