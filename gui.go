...
func secondRowGraph(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	//int(float32(maxY) / 2)
	if v, err := g.SetView("totalPlots", maxX/3+1, maxY/4+1, int(float32(maxX)-1), int(float32(maxY)/4)); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
...
func firstRowGraph(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	//int(float32(maxY) / 2)
	if v, err := g.SetView("main", maxX/2+1, 0, int(float32(maxX)-1), int(float32(maxY)/4)); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
