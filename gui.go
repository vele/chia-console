...
func drawFreeSpaceTable(g *gocui.Gui) error {
	...
	for {
		...
		g.Update(func(g *gocui.Gui) error {
			...
			fmt.Fprintln(v, float64((len(res.Plots) * 108877517724 / 1024 / 1024 / 1024)))
			fmt.Fprintln(v, float64((len(res.Plots) * 102 / 1024)))

			fmt.Fprintln(v, (float64((len(res.Plots) * 108877517724)) / (float64(returnBlockChainDetails()) * (8 * 1000) / float64(8*1000*1000*1000*1000))))
			fmt.Fprintln(v, math.Pow(float64((len(res.Plots)*108877517724)), 3)/1024)
