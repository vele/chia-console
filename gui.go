...
func banner(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	if v, err := g.SetView("banner", 0, int(0.3*float32(maxY)+1), int(float32(maxX)-1.5), int(float32(maxY)/1.1), 0); err != nil {
...
func secondRowGraph(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	//int(float32(maxY) / 2)
	if v, err := g.SetView("totalPlots", int(0.3*float32(maxX)), int(0.3*float32(maxY)), maxX-1, int(0.3*float32(maxY)), gocui.RIGHT); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
