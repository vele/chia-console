...
func plotsLayout(g *gocui.Gui) error {
	...
	if v, err := g.SetView("plots", maxX/2+1, maxY/4+1, int(float32(maxX)/1.5), int(float32(maxY)/2)); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
...
func priceLayout(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	if v, err := g.SetView("chia_price", int(float32(maxX)/1.5)+1, maxY/4+1, int(float32(maxX)-1.5), int(float32(maxY)/2)); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
...
func banner(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	if v, err := g.SetView("banner", 0, int(float32(maxY)/2)+1, maxX-1, int(float32(maxY)/1.5)); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
		v.Title = "Updates"
