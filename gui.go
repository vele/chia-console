...
func drawFreeSpaceTable(g *gocui.Gui) error {
	...
	for {
		...
		g.Update(func(g *gocui.Gui) error {
			...
			chia_probability_formula_0 := (1 - len(res.Plots)*102/1024) / int((float64(returnBlockChainDetails()) * 137438953.472))
			fmt.Fprintln(v, chia_probability_formula_0, (float64(returnBlockChainDetails()) * 137438953.472), float64(len(res.Plots)*102/1024))
			fmt.Fprintln(v, float64(len(res.Plots)*102/1024)/(float64(returnBlockChainDetails())*137438953.472))
			fmt.Fprintln(v, 1-float64(len(res.Plots)*102/1024)/(float64(returnBlockChainDetails())*137438953.472))
			...
			fmt.Fprintf(v, "\u2705 Chance to win chia today :\033[34mUSD %0.5f%%\033[0m\n", chia_probability_formula)
...
func secondRowGraph(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	//int(float32(maxY) / 2)
	if v, err := g.SetView("totalPlots", int(0.3*float32(maxX)), maxY/3+1, maxX-10, maxY-20, 0); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
...
func firstRowGraph(g *gocui.Gui) error {
	maxX, maxY := g.Size()
	if v, err := g.SetView("main", int(0.3*float32(maxX)), 0, maxX-1, maxY/3, 0); err != nil {
		if err != gocui.ErrUnknownView {
			return err
		}
