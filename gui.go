...
func drawFreeSpaceTable(g *gocui.Gui) error {
	...
	for {
		...
		g.Update(func(g *gocui.Gui) error {
			...
			to_bits, _ := new(big.Float).SetPrec(precision).SetString("100")
			...
			netspace_percent := new(big.Float).Mul(part_of_netspace, to_bits)
			...
			fmt.Fprintf(v, "\u2705 Current chia price ( XCH ):\033[93m \uFF04 %f\033[0m\n", chia_price.ChiaPrice)
