...
var instance_detailed InstanceDetail
var first_instance_id string
...
func detailsLayout(g *gocui.Gui) error {
	...
	if v, err := g.SetView("details", 0, maxY/3+1, maxX/4, int(float32(maxY)/1.4)); err != nil {
		...
		if instance_detailed.InstanceId == "" {
			instance_detailed = GetInstanceDetails("eu-north-1", strings.TrimSpace(first_instance_id))
		}
		v.Title = "Disk Details"
		v.Frame = true
		v.Wrap = false
		for _, i := range instance_detailed.VolumeInfo {
			for _, tag := range i.Tags {
				tags = append(tags, fmt.Sprintf("Key: %5s\n Value: %5s\n", *tag.Key, *tag.Value))
			}
			fmt.Fprintf(v, "Volume ID  : %2s\n", *i.VolumeId)
			fmt.Fprintf(v, "Volume IOPS: %2d\n", i.Iops)
			fmt.Fprintf(v, "Volume Size: %2v GB\n", i.Size)
			fmt.Fprintf(v, "Volume Tags: \n%2v\n", tags)
			fmt.Fprintf(v, "Volume State: %2s\n", i.State)
			fmt.Fprintf(v, "Volume Encryption: %2v\n", i.Encrypted)
			fmt.Fprintf(v, "Volume Throughput: %2v\n", i.Throughput)
			fmt.Fprintf(v, "Volume Availability Zone: %2s\n", *i.AvailabilityZone)
			fmt.Fprintf(v, "Volume Create Time: %2v\n", i.CreateTime)
		}
	}
	return nil
}
...
func layout(g *gocui.Gui) error {
	...
	if v, err := g.SetView("instances", 0, 0, maxX-10, maxY/3); err != nil {
		...
		v.Highlight = true
		v.Wrap = false
		v.Title = fmt.Sprintf("%-20s %-50s %-20s %-30s %-20s %-10s ", "Instance ID", "Name", "Instance Type", "Creation Time", "IP", "Security Group")
